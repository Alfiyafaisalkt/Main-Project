...
void main() {
  runApp(const BlindManagement());
}

class BlindManagement extends StatelessWidget {
  const BlindManagement({super.key});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const AddBlind(title: 'ChangePassword'),
    );
  }
}

class AddBlind extends StatefulWidget {
  const AddBlind({super.key, required this.title});

  final String title;

  @override
  State<AddBlind> createState() => _AddBlindState();
}

class _AddBlindState extends State<AddBlind> {
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  final urls = Uri.parse('$url/Blind_Management/');
                  try {
                    ...
                    if (response.statusCode == 200) {
                      ...
                      if (status=='ok') {
                        Fluttertoast.showToast(msg: 'blind Added Successfully');
