...
class MyEdit extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const BlindEdit(title: 'Edit Profile'),
    );
  }
}

class BlindEdit extends StatefulWidget {
  const BlindEdit({super.key, required this.title});

  final String title;

  @override
  State<BlindEdit> createState() => _BlindEditState();
}

class _BlindEditState extends State<BlindEdit> {
  ...
  void _get_data() async{
    ...
    String lid = sh.getString('bid').toString();
    String img_url = sh.getString('img_url').toString();

    final urls = Uri.parse('$url/Blind_single_view/');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (status=='ok') {
          ...
          String phone=jsonDecode(response.body)['contact'].toString();
          String place=jsonDecode(response.body)['place'].toString();
          String post=jsonDecode(response.body)['post'].toString();
          String housename=jsonDecode(response.body)['house name'].toString();
          ...
          housenameController.text=housename;
  ...
  void _send_data() async{
    ...
    String house_name=housenameController.text;
    ...
    final urls = Uri.parse('$url/Edit_blind/');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (status=='ok') {
          ...
          Navigator.push(context, MaterialPageRoute(
            builder: (context) => BlindView(title: "Profile"),));
        }else {
          Fluttertoast.showToast(msg: 'Not Found');
        }
      }
